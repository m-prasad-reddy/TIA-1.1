...
class NameMatchManager:
    """Manages name matches and synonyms for query processing in TableIdentifier-v2.1."""
                    ...
                    for col in schema_dict['columns'][schema][table]:
                        ...
                        for token, token_emb in zip(tokens, token_embeddings):
                            ...
                            if similarity > self.synonym_threshold:
                                suggested = self._suggest_synonym(token, col, schema, table)
                                if suggested:
    ...
    def compute_similarity(self, token: str, column: str) -> float:
        """Compute similarity between token and column."""
        token_emb = self._get_embedding(token)
        col_emb = self._get_embedding(column)
        similarity = util.cos_sim(token_emb, col_emb)[0][0]
        return similarity

    def _suggest_synonym(self, token: str, column: str, schema: str, table: str) -> bool:
        """Prompt user to confirm a synonym, only once per session."""
        try:
            ...
            self.logger.error(f"Error prompting for synonym '{token}': {e}")
            return False
    ...
    def suggest_synonyms(self, column: str, schema_dict: Dict) -> List[str]:
        """Suggest synonyms for a given column."""
        try:
            ...
            self.logger.error(f"Error suggesting synonyms for '{column}': {e}")
            return []

    def extract_tokens(self, query: str) -> List[str]:
        """Extract relevant tokens from query."""
        doc = self.nlp(query.lower())
